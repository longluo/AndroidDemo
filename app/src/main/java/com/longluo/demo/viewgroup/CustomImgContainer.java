...
public class CustomImgContainer extends ViewGroup {
	...
	/*
     * if (heightMode == MeasureSpec.UNSPECIFIED) { int tmpHeight = 0 ;
	 * LayoutParams lp = getLayoutParams(); if (lp.height ==
	 * LayoutParams.MATCH_PARENT) { Rect outRect = new Rect();
	 * getWindowVisibleDisplayFrame(outRect); tmpHeight = outRect.height();
	 * }else { tmpHeight = getLayoutParams().height ; } height =
	 * Math.max(height, tmpHeight);
	 * 
	 * }
	 */
}
