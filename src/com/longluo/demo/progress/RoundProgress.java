...
public class RoundProgress extends View {
	...
	private void init() {
		...
		mWidth = mHeight = (int) (2 * (mCenterCircleRadius + 2
				* mWallCircleRadius * (1 + mMaxStaticCircleRadiusScaleRate)));
		...
		mActiveCircle.y = mWallCircleRadius
				* (1 + mMaxStaticCircleRadiusScaleRate);

		mWallCircles = new Circle[mWallCircleCount];
		for (int i = 0; i < mWallCircleCount; i++) {
			Circle circle = new Circle();
			circle.angle = 360 / mWallCircleCount * i;
			circle.x = (float) (mCenterCircle.x + (mCenterCircleRadius + mWallCircleRadius)
					* Math.cos(Math.toRadians(circle.angle)));
			circle.y = (float) (mCenterCircle.y + (mCenterCircleRadius + mWallCircleRadius)
					* Math.sin(Math.toRadians(circle.angle)));
	...
	private boolean doAdhere(Circle circle) {
		/* 半径变化 */
		float distance = (float) Math.sqrt(Math.pow(mActiveCircle.x - circle.x,
				2) + Math.pow(mActiveCircle.y - circle.y, 2));// Math.abs(mDynamicCircle.x
																// - circle.x);
	...
	private void startAnim() {
		ValueAnimator valueAnimator = ValueAnimator.ofFloat(
				mActiveCircle.angle, mActiveCircle.angle + 360);
	...
	private void drawCircle(Canvas canvas) {
		canvas.drawCircle(mActiveCircle.x, mActiveCircle.y,
				mActiveCircle.radius, mPaint);
		...
		for (int i = 0; i < mWallCircleCount; i++) {
			...
			if (doAdhere(circle)) {
				...
				Path path = drawAdherentBody(mActiveCircle.x, mActiveCircle.y,
						mActiveCircle.radius, 45f, circle.x, circle.y,
						circle.radius, 45f);
