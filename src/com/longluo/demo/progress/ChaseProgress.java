...
public class ChaseProgress extends View {
	...
	private void init() {
		...
		for (int i = 0; i < mWallCircleCount; i++) {
			...
			circle.x = (float) (mCenterCircle.x + (mCenterCircleRadius
					+ mDistance + mWallCircleRadius)
					* Math.cos(Math.toRadians(circle.angle)));
			circle.y = (float) (mCenterCircle.y + (mCenterCircleRadius
					+ mDistance + mWallCircleRadius)
	...
	private void startAnim() {
		...
		for (int i = 0; i < mWallCircleCount; i++) {
			...
			ValueAnimator valueAnimator = ValueAnimator.ofFloat(circle.angle,
					circle.angle + 360);
			valueAnimator
