...
public class SkipProgress extends View{
	...
	private void init(){
		...
		mWidth = (int)((mWallCircleCount-1)*mDistance+2*mWallCircleCount*mWallCircleRadius); 
		...
		for(int i=0;i<mWallCircleCount;i++){
			...
			circle.x = mWallCircleRadius+i*(2*mWallCircleRadius+mDistance);
	...
	protected void onDraw(Canvas canvas) {		
        for(int i=0;i<mWallCircleCount;i++){        	
        	canvas.drawCircle(mWallCircles[i].x, mWallCircles[i].y, mWallCircles[i].radius, mPaint);
        }
	}
	...
	private void startUpAnim(final int pos){
		...
		ValueAnimator valueAnimator = ValueAnimator.ofFloat(mHeight-mWallCircleRadius,mWallCircleRadius);
	...
	private void startDownAnim(final int pos){	
		if(pos+1<mWallCircleCount) startUpAnim(pos+1);
		final Circle circle = mWallCircles[pos];
		ValueAnimator valueAnimator = ValueAnimator.ofFloat(mWallCircleRadius,mHeight-mWallCircleRadius);
		valueAnimator.setInterpolator(new AccelerateDecelerateInterpolator());
		valueAnimator.setDuration(500);				
		valueAnimator.addUpdateListener(new AnimatorUpdateListener() {
		...
		valueAnimator.addListener(new AnimatorListenerAdapter() {
			...
			public void onAnimationEnd(Animator animation) {
				super.onAnimationEnd(animation);    
				if(pos+1==mWallCircleCount) startUpAnim(0);
			}					
		});
		valueAnimator.start();
	}
